...
const Record = (props) => {
  const recordCards = props.records.map(record => { 
    return (<a href="/" onClick={preventEventDefault} className={`ui card store${record.storeNumber}`} id="card" key={record.id} style={{margin: "10px auto", textDecoration: "none", color: "black", width: "95%"}}>
